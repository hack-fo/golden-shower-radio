...
1. **Store & schema (Group KS).** A persisted, relational, queryable knowledge store exists in
   ...
   library by artist/title keying.
...
3. **Continuous research jobs (Group KR).** New-artist ingest (ANALYSIS-006 REQ-AP-007), stale-flag
   refresh, and pre-show prep enqueue bounded, throttled, rate-limit/ToS-respecting research jobs
   ...
   gracefully on outage/quota (Scenario B-2); no research touches the pull path.
...
5. **Grounding feed & integration (Group KI).** The knowledge base is the verified-facts source for
   ...
   recorded to the ledger/diary substrate.
...
7. **NFRs.** Dated/sourced facts (NFR-K-1); never-stale + never-unconfirmed-as-certain +
   ...
   comparisons grounded in real edges (NFR-K-6); simplicity/no over-engineering (NFR-K-7) all hold
   and are verified.
