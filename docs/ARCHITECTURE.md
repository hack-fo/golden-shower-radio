...
Docker Compose topology, the pull-based playout loop, the brain's internal subsystems,
data stores, the Claude Max subscription auth model, and the SPEC suite.
...
**The brain never receives `ANTHROPIC_API_KEY`.** The Compose file passes only an
explicit allowlist of environment variables to the brain container. If `ANTHROPIC_API_KEY`
reached the brain it would silently override the Claude Max subscription and bill
pay-per-use credits — the failure mode that broke an earlier brain. As a second defence,
`brain/main.py` pops the key from the environment immediately on startup and
`brain/llm.py` strips it from subprocess envs. See [Curation Director](components/curation-director.md)
for the full auth contract.

**slskd is behind a Compose profile and the brain does not depend on it.** On networks
that block P2P traffic — or by user preference — the profile is simply omitted and the
brain falls through to yt-dlp for every acquisition. The brain has no `depends_on: slskd`.
As of 2026-06-22 slskd is **disabled by default**. See [Acquisition](components/acquisition.md).
...
that: **Liquidsoap pulls**, asking the brain what to play on every track boundary.
This is the central architectural decision and it lives in `deploy/config/radio.liq`.
...
2. `next_track()` does `http.get("http://brain:8080/api/next")`. The brain returns a
   ...
   ```
3. `request.create` resolves the trailing file path and applies the annotated metadata,
   **overriding the file's embedded tags** — so players always show the brain's clean
   artist/title even when a file's tags are garbled.
4. The source prefetches **2 items ahead** (`prefetch=2`) so the crossfade has tails to
   work with. This means `/api/next` is called up to 2 tracks before air.
5. A per-kind **transition** function branches on `mix_mode`:
   - `music → music`: `add([fade.out(3s, old), fade.in(3s, new)])` — an **unconditional**
     3-second overlap. `cross.smart` is deliberately **not** used; its dB heuristic can
     decide to hard-cut, which the design rules out.
...
6. When an item **actually starts on air**, `radio.on_metadata(report_airing)` fires and
   POSTs the track back to `/api/airing` in a background thread. The brain sets its
   displayed now-playing from *that* callback — not from `/api/next`.
...
"handed out" as "now playing". `on_metadata` (not `on_track`, which `cross` deduplicates
unreliably at fade boundaries) gives the brain the **ground truth** of what listeners are
hearing. The brain commits rotation and cadence at hand-out time but sets the displayed
track only from airing reports — so the website never leads the broadcast.
...
`brain.main.run()`. `main.py` wires up all subsystems and launches each as a resilient
daemon thread (catch, log, continue), with graceful shutdown on SIGINT/SIGTERM.
...
| Method + path | Purpose |
|---------------|---------|
| `GET /api/next` | Returns the next item as a Liquidsoap `annotate:` URI, or empty `200` if nothing is ready. **Commits** rotation/cadence. Must respond in `<1s`. |
...
The `/api/next` budget is `<1 s`. The request path only reads state and commits. Every
...
The brain persists state under `/db` (Docker bind-mounted from the gitignored `data/db/`):
...
them, and under `MUSIC_DIR` so Liquidsoap can read them through the existing read-only
volume mount without a new volume.

---

## Claude Max subscription auth model

The brain uses the **Claude Max subscription** — OAuth credentials stored in a mounted
`~/.claude` directory — not a pay-per-use API key. This is the single most important
billing constraint in the system.

The rule is simple: `ANTHROPIC_API_KEY` must **never** reach the brain container. If it
does, the Claude CLI switches to pay-per-use mode and will exhaust credits immediately.
Three defences are in place:

1. `deploy/docker-compose.yml` passes only an explicit env allowlist to `gsr-brain`; `ANTHROPIC_API_KEY` is not on it.
2. `brain/main.py` pops `ANTHROPIC_API_KEY` from `os.environ` as the very first act of startup.
3. `brain/llm.py` strips the key from subprocess envs before every `claude` CLI call.

Every LLM call is configured with:
- `system_prompt` as a plain string (avoids loading the ~85K-token `claude_code` preset)
...
Changing any of these — especially `system_prompt` to the `claude_code` preset — would
burn the 5-hour subscription quota in minutes.

---

## Resilience model

The station's identity is **continuous operation**, not a zero-gap real-time guarantee:

- Every worker loop is wrapped: a failed tick logs and continues; the loop never crashes.
- `llm.py` never raises — on any SDK error, quota limit, rate limit, or parse failure it returns the built-in seed list (curation) or `""` (talk, the break is skipped).
- `knowledge.py` never raises into callers — a store error degrades knowledge richness, never playout continuity.
- `voice.py` auto-falls back: Kokoro → Piper → skip-the-break.
- `/api/next` returns an empty `200` on any error; Liquidsoap retries after `retry_delay=2.0 s`.
